...
vs = PiVideoStream(resolution = (640,320)).start(brightness = 60,contrast = 100)
time.sleep(2.0)
greenLower = (12, 86, 40)
...
try:
	while True:
		...
		circles = cv2.HoughCircles(mask.copy(),cv2.HOUGH_GRADIENT,1,20,
		...
		if len(cnts) > 0:
			...
			if radius > 10 and radius < 30:
				print(center)
				if abs(500-center[0]) < 40 and False:
					activated = 1
					robot.Stop()
					robot.FlyWheelsOn()
					robot.Forward()
